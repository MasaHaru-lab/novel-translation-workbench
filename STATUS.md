...
### Phase B Real-Model Validation (2026-05-02)

**Status:** VALIDATED. Round 2 (notes-variant filtering, HEAD `9f87a6f`)
and Round 3/Phase B Round 2 (cross-entity prevention, HEAD `eaeefa4`)
hold against real-model output with DeepSeek profile and full book memory
context. No remaining mechanical consistency pollution.

**Run:** `venv/bin/python -m app.cli chapter run --source data/source/one_chapter_quality_source.txt --book-memory data/book_memory/book_memory.json --model-profile deepseek-v4-flash`

**Result:** 3/3 segments, 0 retries, quality gate passed (0 errors, 0 warnings).
All 6 inspection priorities (legal mother, Chi Yuan Daoist, Old Lady Qin vs old
lady, Qin Liuxi/Xi'er, cross-entity, context-sensitive terms) — clean. No Round 4
fix batch needed.

**Closeout record:** `docs/phase_b_validation_closeout.md`
